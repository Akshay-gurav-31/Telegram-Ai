...
MOCK_API_URL = "https://jsonplaceholder.typicode.com/posts"  # Mock API for fallback
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        if USE_MOCK_API:
            # Use JSONPlaceholder mock API
            response = requests.get(MOCK_API_URL)
            response.raise_for_status()
            result = response.json()
            ai_response = result[0]["title"] if result else "Mock API response"
